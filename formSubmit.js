...
const formSubmit = new function() {
  ...
  const self = this,
    ...
    const initialize = () => {             
        ...
        forms.forEach(form => {
            const inputs = form.querySelectorAll('input:not([type=button]):not([type=reset]):not([type=submit]):not([type=image]),select,textarea');
            for(let i = 0; i < inputs.length; i++) {addCounter()}
            inputs.forEach(input => {
              const { type, dataset } = input
              const { type: dataType,} = dataset
              const resolvedType = (dataType || type) === 'true' ? (dataType || type).toLowerCase() : (dataType || type).toLowerCase()//favor dataset value over native type (to allow for applying 'custom' application of formSubmit checks)
              if( !['file','image','hidden','select-one','select-multiple','radio','checkbox'].includes(resolvedType) ) {
                ...
                addMultipleEventListeners(input, ['input','change','blur'], handleInput)
                input.addEventListener('keydown', handleInputKeys)
              }
              if(['file','image'].includes(resolvedType)){
                addMultipleEventListeners(input, ['input','change','blur'], handleFileInput)
              }
              if(['select-one','select-multiple'].includes(resolvedType)) {
                addMultipleEventListeners(input, ['input','change','blur'], handleSelect)
              }
              if(resolvedType === 'radio') {
                ...
                addMultipleEventListeners(input, ['input','blur'], handleRadiosCheckboxes)
              }
              if(resolvedType === 'checkbox') {
                addMultipleEventListeners(input, ['input','change','blur'], handleRadiosCheckboxes)
              }
            })
        ...
        self.checkAllValidity = () => {
          ...
          forms.forEach(form => {
            ...
            inputs.forEach(input => {
              ...
              types[resolvedType].value.test(input.value) ? validInputs.push(input) : invalidInputs.push(input)
            });
          });
        ...
        self.isValid = (el) => {
          ...
          const type = (el.dataset.type || el.type).toLowerCase() === 'true' ? 'text' : (el.dataset.type || el.type).toLowerCase()
          if(tag === 'FORM') {//Allows for passing in an entire FORM tag and validating all inputs where as checkAllValidity checks all forms on the page
            const inputs = el.querySelectorAll('input:not([type=button]):not([type=reset]):not([type=submit]):not([type=image]),select,textarea')
            inputs.forEach(input => {
            ...
            return (!type) ? console.error(`Invalid Element Type: ${tag}\r\nValid input elements only`) :
            (['select-one','select-multiple'].includes(type)) ? handleSelect(el, true) :
            (['radio','checkbox'].includes(type)) ? handleRadiosCheckboxes(el,true) : 
            (!['select-one','select-multiple'].includes(type)) ? handleInput(el, true) : false
          }
        }
        ...
        const handleFormSubmit = (e) => {
          ...
          const inputs = e.target.querySelectorAll('input:not([type=button]):not([type=reset]):not([type=submit]):not([type=image]),select,textarea');//selects all form input elements
          inputs.forEach(input => input.value = input.value.trim())//Trims leading and trailing spaces
          document.querySelectorAll('form').forEach(form=>{finalCount += form.querySelectorAll('input:not([type=button]):not([type=reset]):not([type=submit]):not([type=image]),select,textarea').length});
          (Array.from(inputs).filter(input => !formSubmit.isValid(input))).length > 0 && (initCount === finalCount) ? e.preventDefault() : null
        }
        const handleSelect = (e, cb = null) => {
          ...
          const val = Array.from(input.children).filter(child => child.selected).map(child => child.value.trim()).join(',');
          const valid = required ? (val ? true : false) : true
          if(!valid && !cb) {
            if(!input.parentNode.querySelector(errClassSelector)) {appendErrEl(input)}//if there is no error message field, create it
          ...
          if(valid && !cb) {
            if(input.parentNode.querySelector(errClassSelector)) {input.parentNode.querySelector(errClassSelector).remove()}//Remove the error field if there is no error, reduces impact to CSS
          }
        ...
        const handleRadiosCheckboxes = (e, cb = null) => {
          ...
          const controlGroup = target.parentNode.parentNode.tagName.toLowerCase() !== 'form' && target.parentNode.parentNode.querySelectorAll(`input[type=${type}]`).length !== 0 ? target.parentNode.parentNode.querySelectorAll(`input[type=${type}]`) : target.parentNode.querySelectorAll(`input[type=${type}]`)
          const container = target.parentNode.parentNode.tagName.toLowerCase() !== 'form' && target.parentNode.parentNode.querySelectorAll(`input[type=${type}]`).length !== 0 ? target.parentNode.parentNode : target.parentNode
          ...
          const valid = isRequired ? (Array.from(controlGroup).filter(el => el.checked === true).length > 0 ? true : false) : true//if required is present on any element in the group, check that a selection is made 
          if(!valid && !cb) {
            if(!container.querySelector(errClassSelector)) {appendErrEl(container, type)}
          ...
          if(valid && !cb) {
            if(container.querySelector(errClassSelector)) {container.querySelector(errClassSelector).remove()}//Remove the error field if there is no error, reduces impact to CSS
          }
          if(cb) return valid;
        }
        ...
        const controlRadios = (e) => {
          const target = e.target || e;//allows for event driven and passing in entire elements
          //checked for radios contained in 1 element or contained in multiple wrapping elements          
          const radioGroup = target.parentNode.parentNode.tagName.toLowerCase() !== 'form' && target.parentNode.parentNode.querySelectorAll('input[type=radio]').length !== 0 ? target.parentNode.parentNode.querySelectorAll('input[type=radio]') : target.parentNode.querySelectorAll('input[type=radio]')
          //Forces only one radio selection for each group of radio buttons
          Array.from(radioGroup).filter(el => el !== target).forEach(el => el.checked = false)          
        }
        ...
        const handleInput = (e, cb = null) => {
            const input = e.target || e; //allows for event driven and passing in entire elements
            const inputType = (input.dataset.type || input.type) === 'true' ? 'text' : (input.dataset.type || input.type)//favor dataset value over native type (to allow for applying 'custom' application of formSubmit checks)
            ...
            if(!cb){
              ...
              required && isNull ? (inputErrorField.textContent = errMsg) ://if required and has no value 
              required && !isNull && !types[inputType].value.test(input.value) ? (inputErrorField.textContent = errMsg) : //if required and has an invalid value
              !required && !isNull && !types[inputType].value.test(input.value) ? (inputErrorField.textContent = errMsg) : inputErrorField.textContent = '' //if not required but has an invalid value
              //Remove the error field if there is no error, reduces impact to CSS
              if(inputErrorField.textContent === '') inputErrorField.remove();
            }
            if(cb) return required && isNull ? false : required && !isNull && !types[inputType].value.test(input.value) ? false : !required && !isNull && !types[inputType].value.test(input.value) ? false : true
        }
          ...
          if (!cb) {
            ...
            if(input.files.length > 0) {
              ...
              for(const file of files) {
                ...
                const isValidType = acceptedTypes === '*' || acceptedTypes[0] === '*' ? true : acceptedTypes.some(atype => atype.endsWith("/*") ? type.startsWith(atype.slice(0, -2)) : type.includes(atype.replace('.', '')))
                !isValidType && fileTypeErr.push(`${name} - Invalid file type, ${type.toLowerCase()}.`)
                regexCheck.push(types.file.value.test(name))
              }
              required && input.files.length === 0 ? (inputErrorField.textContent = errMsg) :
              required && input.files.length > 0 && fileTypeErr.length > 0 ? (inputErrorField.textContent = fileTypeErr.join('\r\n')) :
              !required && input.files.length > 0 && fileTypeErr.length > 0 ? (inputErrorField.textContent = fileTypeErr.join('\r\n')) : inputErrorField.textContent = ''
              if (fileTypeErr.length > 0) input.value = '';
            }
            inputErrorField.textContent === '' && inputErrorField.remove();
          }
          if(cb) return required && input.files.length === 0 ? false : required && input.files.length > 0 && !regexCheck.includes(false) ? false : !required && input.files.length > 0 && !regexCheck.includes(false) ? false : true
        }
        ...
        const handleInputKeys = (e) => {
            ...
            const type = (e.target.dataset.type || e.target.type) === 'true' ? 'text' : (e.target.dataset.type || e.target.type) //sets the input type
            if(!(types[type].keys.test(key))){e.preventDefault();} //passes key code through regex of allowed keys, if not allowed, no action is taken
        }
        ///////////////////////////Helper Functions///////////////////////////////////////////////////
        ...
        const removeAllEventListeners = (input) => {
          ...
          const handlers = [handleInputKeys, handleInput, handleFileInput, handleSelect, handleRadiosCheckboxes, controlRadios];        
        ...
        if(document.readyState === 'complete') {initialize();}//allows for defer/"lazy" loading
}